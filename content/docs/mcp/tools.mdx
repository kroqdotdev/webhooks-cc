...
| `mockResponse` | `object`  | no       | Mock response config with `status`, `body`, `headers`, `delay?` (ms, 0-30000) |
...
| `mockResponse` | `object` | no       | Mock response config `{ status, body, headers, delay? }`, or `null` to clear |
