...
seo:
  ...
  ogDescription: "Verify webhook signatures for 14 providers using the webhooks.cc TypeScript SDK. Supports HMAC-SHA256, HMAC-SHA1, Ed25519, and token comparison."
---

## Overview

Most webhook providers sign payloads so you can verify authenticity before processing. The SDK includes verification functions for 14 providers (13 with cryptographic verification, plus SendGrid which uses IP allowlisting), plus a generic `verifySignature` that dispatches by provider name.
