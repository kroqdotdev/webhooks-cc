...
description: "Filter, detect, and parse webhook requests with built-in matchers for method, header, and body matching. Includes provider detection helpers and JSON body parsing."
section: sdk
order: 4
...
seo:
  ogTitle: "Matchers & Helpers"
  ogDescription: "Filter, detect, and parse webhook requests with built-in matchers for method, header, and body matching. Includes provider detection helpers and JSON body parsing."
---
...
Requires [signature verification](/docs/dashboard-features#signature-verification) to be configured on the endpoint. Returns `false` for requests where verification was not configured (`signatureVerified` is `null`).
...
Detector functions identify common providers from headers and, for providers without a distinctive signature header (SendGrid, Meta, Lemon Squeezy, Mailgun, Adyen), body shape. Use `detectWebhookInfo` when you want the provider and event/topic, or the boolean helpers when you only need to route one provider.
...
| Function                             | Match source                                                 | Event source                                                 |
| ------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
...
| `isDocuSignWebhook(request)`         | `x-docusign-signature-1`                                     | JSON `event`                                                 |
| `isAdyenWebhook(request)`            | JSON `notificationItems.0.NotificationRequestItem.eventCode` | JSON `notificationItems.0.NotificationRequestItem.eventCode` |
| `isPayPalWebhook(request)`           | `paypal-transmission-sig`                                    | JSON `event_type`                                            |
| `isPlaidWebhook(request)`            | `plaid-verification`                                         | JSON `webhook_code`                                          |
...
<LinkCard
  ...
  description="Verify webhook signatures for 30 signed providers."
/>
