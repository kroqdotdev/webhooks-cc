...
- **Customer:** per team, created lazily before first checkout: `polar.customers.create({ type: "team", name: team.name, externalId: "team:<teamId>", metadata: { teamId }, owner: { email: owner.email, name: owner.name, externalId: owner.id } })`. The customer carries no email of its own: Polar allows one customer per email per organization, and the owner is usually already one (personal Pro) or owns another team, so the owner rides along as the owner member (Polar prefills checkout, receipts, and portal sign-in from it; member emails are only unique per customer). Keeps team webhook routing unambiguous and avoids Polar permanently flagging the owner's personal customer as a `team`-type customer. (Changed 2026-08-21 after the prod checkout smoke test failed with "A customer with this email address already exists"; v0.28.1.)
