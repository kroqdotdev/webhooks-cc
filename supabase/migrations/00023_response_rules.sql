-- ============================================================================
-- Migration 00023: Conditional mock response rules
--
-- Add response_rules JSONB column to endpoints for ordered conditional
-- response matching. Update capture_webhook() to return response_rules
-- alongside mock_response so the Rust receiver can evaluate rules.
--
-- Rules are evaluated in the receiver (Rust), not in SQL.
-- The stored procedure only passes the rules blob through.
-- ============================================================================

-- 1. Add nullable JSONB column (metadata-only, no table rewrite)
alter table public.endpoints
  add column if not exists response_rules jsonb;

-- 2. Replace capture_webhook with version that returns response_rules
...
begin
  ...
  -- 1. Look up endpoint by slug (now includes response_rules)
