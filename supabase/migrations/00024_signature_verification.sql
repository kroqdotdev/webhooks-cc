-- ============================================================================
-- Migration 00024: Signature verification
--
-- Add server-side webhook signature verification. Endpoints can store an
-- encrypted signing secret + provider. The Rust receiver decrypts the secret
-- post-response and verifies signatures, storing the result on the request row.
--
-- Changes:
--   endpoints: +signing_provider, +signing_secret_encrypted, +signing_header
--   requests:  +signature_verified, +signature_error, +signing_provider
--   capture_webhook(): returns request_id + signing fields for post-capture verification
-- ============================================================================

-- 1. Endpoints: signing configuration columns
...
-- 3. Index for "show me all failed verifications for this endpoint"
create index if not exists idx_requests_signature_verified
  on public.requests (endpoint_id, signature_verified, received_at desc)
  where signature_verified is not null;

-- 4. Update capture_webhook() to return request_id + signing fields
...
begin
  ...
  insert into public.requests (
    endpoint_id, user_id, method, path, headers, body, body_raw,
    query_params, content_type, ip, size, received_at
  ) values (
    v_endpoint.id, v_endpoint.user_id, p_method, p_path, p_headers, p_body, p_body_raw,
