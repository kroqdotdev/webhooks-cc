-- ============================================================================
-- Migration 00019: Raw body fidelity
--
-- Add body_raw bytea column for byte-exact storage of non-UTF-8 payloads.
-- Update capture_webhook() with optional 10th parameter p_body_raw.
--
-- The text body column is preserved for display/search. body_raw is only
-- populated when the original payload is not valid UTF-8 (the receiver
-- detects this and sends both the lossy text and the raw bytes).
-- ============================================================================

-- 1. Add nullable bytea column (metadata-only, no table rewrite)
alter table public.requests add column if not exists body_raw bytea;

-- 2. Replace capture_webhook with 10-param version
...
begin
  ...
  -- Prefer raw byte length when available for accurate size
